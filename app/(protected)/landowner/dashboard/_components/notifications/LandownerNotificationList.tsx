// app/(protected)/landowner/dashboard/_components/notifications/LandownerNotificationList.tsx
'use client';
...
export const LandownerNotificationList: React.FC = () => {
  const { user } = useUser();
  ...
  const handleNotificationClick = async (notification: Notification) => {
    ...
    switch (notification.type) {
      ...
      case 'LISTING':
        if (notification.entityId) {
          router.push(`/landowner/listings/${notification.entityId}`);
        }
        break;
      default:
          ...
          {notifications.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-8 text-center">
              ...
              <p className="text-xs text-muted-foreground">
                When farmers interact with your listings, you&apos;ll see it here
              </p>
            </div>
          ) : (
